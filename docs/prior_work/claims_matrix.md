...
| P2-C12 | Even-gap measures decay asymptotically geometrically. | `CONDITIONAL_THEOREM` using spectral ingredients | The physical adjacent-mass ratio limit is `PROVED` analytically. The explicit rate `|mu(C_(2n+2))/mu(C_(2n))-U_c^2/4| < (243/625)*(3/5)^(n-6)` for `n>=6` and selected absolute-mass enclosures are `NUMERICALLY_CERTIFIED`. The legacy ordinary-`BV` proof is `REFUTED`, and any stronger finite-order mass formula not implied by this rate remains `OPEN`. | Corrected physical adjacent-ratio theorem only | Cite the direct density, sharp-cone, and cusp-rate proofs; do not restore the old spectral argument, claim exact finite-`n` weights, or infer a determinant |
