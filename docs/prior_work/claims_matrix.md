...
| P2-C12 | Even-gap measures decay asymptotically geometrically. | `CONDITIONAL_THEOREM` using spectral ingredients | Ratio-limit subclaim `PROVED` for the physical exact-$U_c$ acip; a validated sharp-cone enclosure now certifies `0.20655<h(0)<0.40008` and selected absolute masses, while the stronger geometric asymptotic with exponential remainder remains `OPEN` and the legacy ordinary-`BV` proof is `REFUTED` | Corrected physical mass-ratio theorem only | Cite the direct density and sharp-cone proofs; do not claim the old exponential remainder, exact finite-$n$ weights, or a determinant |
